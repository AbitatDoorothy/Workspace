...
- [ ] `abitat iphone` starts a Mac-side Cloudflare Quick Tunnel when no transport is specified.
- [ ] `abitat iphone` prints a QR code and manual pairing payload with a `trycloudflare.com` endpoint.
...
- [ ] Core mobile control succeeds without `workspace.abitat.io`, a hosted account, or a hosted database.
